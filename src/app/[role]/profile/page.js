...
import {
  ...
  Divider,
  OutlinedInput,
  Avatar, // Added Avatar for profile picture display
  Stack,  // Added Stack for layout of avatar and upload button
} from "@mui/material";
import { Grid } from "@mui/material";
import { expertAPI, organisationAPI, programmeAPI, volunteerAPI, studentAPI, userAPI } from "@/lib/api";
import { useSession } from "next-auth/react";
import ProfilePictureUpload from "@/app/components/ProfilePictureUpload"; // Import the new component
...
const ProfilePage = () => {
  ...
  const [submitting, setSubmitting] = useState(false); // For form submission
  const [currentUserId, setCurrentUserId] = useState(null); // Stores the ID of the user being edited
  const [userImage, setUserImage] = useState(null); // New state for user image URL
      ...
      try {
        ...
        if (session?.user?.id) { // Check if user ID exists before attempting to fetch
          ...
          if (userResponse.success && userResponse.data) {
            ...
            const profileData = user.profile;
            setCurrentUserId(user.profile.id); // Set the current user ID for updates
            setUserImage(user.image || null); // Set user image from user table
  ...
  const handleProfilePictureUploadComplete = async (imagePath) => {
    setSubmitting(true); // Indicate that a submission is in progress
    try {
      if (!session?.user?.id) {
        throw new Error("User ID not found in session for image update.");
      }

      // Update the user's image in the users table
      const response = await userAPI.update(session.user.id, {
        image: imagePath,
        name: formData.name, // Include name and email as per requirement
        email: formData.email,
        phoneNumber: formData.contactNumber,
      });

      if (response.success) {
        setUserImage(imagePath); // Update local state with new image
      ...
      setSnackbarMessage(`Error updating profile picture: ${error.message || 'Unknown error'}`);
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
    } finally {
      setSubmitting(false); // End submission indication
    }
  };

  const handleProfilePictureUploadError = (errorMessage) => {
    setSnackbarMessage(`Profile picture upload failed: ${errorMessage}`);
    setSnackbarSeverity("error");
    setSnackbarOpen(true);
  };
          ...
          <Grid size={12}>
            <Paper elevation={1} sx={{ p: 3, mb: 3, borderRadius: 2 }}>
              ...
              <Grid container spacing={3} alignItems="center">
                {/* Profile Picture/Avatar and Upload */}
                <Grid size={{ xs: 12, sm: 6, md: 4 }} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  <Avatar
                    src={userImage}
                    sx={{
                      width: 120,
                  ...
                  >
                    {!userImage && getInitials(formData.name)}
                  </Avatar>
                  {(role === 'volunteer' || role === 'expert') && (
                    <ProfilePictureUpload
                      ...
                      disabled={submitting}
                      buttonText={userImage ? "Change Profile Picture" : "Upload Profile Picture"}
                    />
                  )}
                </Grid>

                {/* Existing TextFields for Basic Info */}
                  ...
                  <Grid container spacing={3}>
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <TextField
                        ...
                        onChange={(e) => handleInputChange("name", e.target.value)}
                        required
                        disabled={role === 'student'} // Disabled for student
                      />
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <TextField
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                      {renderDropdownField(
                        ...
                        'id',
                        'name',
                        true // Disable for all roles as it's an assigned field
                      )}
                    </Grid>
