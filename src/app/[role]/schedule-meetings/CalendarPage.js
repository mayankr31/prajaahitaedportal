  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (activitiesResponse.success) {
        // Filter activities based on approval status for calendar display
        // Only APPROVED activities should be displayed on the calendar for all roles
        const approvedActivities = activitiesResponse.data.filter(
          (activity) => activity.approvalStatus === "APPROVED"
        );
        const transformedActivities =
          transformActivitiesToEvents(approvedActivities);
  ...
  const transformMeetingsToEvents = (meetingsData) => {
    return meetingsData.map((meeting) => {
      ...
      if (meeting.repeat && meeting.repeat !== "Does not repeat") {
        const rruleConfig = generateRRule(
          meeting.repeat,
    ...
    return activitiesData.map((activity) => {
      ...
      return {
        ...
        extendedProps: {
          ...
          rejectionMessage: activity.rejectionMessage,
        },
        color: "#8B5CF6",
      };
    });
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {selectedEvent.type === "meeting" && (
                  <>
                    ...
                    {selectedEvent.repeat &&
                      selectedEvent.repeat !== "Does not repeat" && (
                        <span className="inline-block px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
                          {selectedEvent.repeat}
                        </span>
                      )}
                    {selectedEvent.videoCall &&
                      selectedEvent.videoCall !== "none" && (
                ...
                {/* Add approval status for activities */}
                {selectedEvent.type === "activity" && (
                  <span
                    className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${
                      selectedEvent.approvalStatus === "APPROVED"
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-gray-600">{formatDate(selectedEvent.start)}</p>
              {!selectedEvent.allDay && (
              ...
              {selectedEvent.allDay && <p className="text-gray-600">All Day</p>}
            </div>
            ...
            {selectedEvent.type === "meeting" && (
              <>
                ...
                {(selectedEvent.createdBy ||
                  ...
                  selectedEvent.rejectionMessage) && (
                  <div className="bg-gray-50 rounded-lg p-4">
                    {selectedEvent.createdBy && (
                      <Typography
                        variant="body2"
                      ...
                      >
                        ...
                        {selectedEvent.createdBy?.email || "N/A"}) -{" "}
                        {selectedEvent.createdBy?.role || "N/A"}
                      </Typography>
                    )}
                    {selectedEvent.approvedBy && (
                      <Typography
                      ...
                      >
                        ...
                        {selectedEvent.approvedBy?.name || "Unknown"} (
                        {selectedEvent.approvedBy?.email || "N/A"}) -{" "}
                        {selectedEvent.approvedBy?.role || "N/A"}
                      </Typography>
                    )}
                    {selectedEvent.rejectionMessage &&
                    ...
                    <h4 className="font-semibold text-gray-700 mb-2 flex items-center">
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin="round"
                          strokeWidth={2}
                          d="M4 16l4.586-4.586a2 2 0 012.828 0L20 20m-6-6l-2 2m2-2l-2-2m7-2h-2m-2 0H5V5h14v14z"
                        />
                      </svg>
                      Image
                    </h4>
                    ...
                    <h4 className="font-semibold text-gray-700 mb-2 flex items-center">
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin="round"
                          strokeWidth={2}
                          d="M7 8h10M7 12h4m1 8h-4a2 2 0 01-2-2V5a2 2 0 012-2h10a2 2 0 012 2v10a2 2 0 01-2 2z"
                        />
                      </svg>
                      Feedback
                    </h4>
            ...
            {/* Created By / Approved By / Rejection Message for activities */}
            {selectedEvent.type === "activity" &&
              (selectedEvent.createdBy ||
                selectedEvent.approvedBy ||
                selectedEvent.rejectionMessage) && (
