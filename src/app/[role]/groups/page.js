...
"use client";
import React, { useState, useEffect } from "react";
import { Plus, ChevronRight, CheckCircle, XCircle, Trash2 } from "lucide-react";
import { useRouter, useParams } from "next/navigation";
import AddGroupDialog from "./AddGroupDialog";
import ApprovalGroupsDialog from "./ApprovalGroupsDialog";
import { groupAPI } from "@/lib/api";
import { useSession } from "next-auth/react";
...
const Groups = () => {
  ...
  const canDeleteGroup = (group) => {
    // Admins and experts can delete any group
    if (session?.user?.role === "admin" || session?.user?.role === "expert") {
      return true;
    }

    // Volunteers can only delete their own pending or rejected groups
    if (session?.user?.role === "volunteer") {
      return (
        group.createdById === session.user.id &&
        (group.approvalStatus === "PENDING" ||
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        console.log("Group deleted successfully:", response.message);
      } else {
  ...
  const renderGroupCard = (group) => (
    <div key={group.id} className="flex flex-col items-center">
      <div
        className="bg-white rounded-xl h-56 w-56 relative shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden cursor-pointer group"
        onClick={() => {
          if (group.approvalStatus === "APPROVED") handleGroupClick(group.id);
          else return;
        }}
      >
        {/* Delete Button */}
        {canDeleteGroup(group) && (
          <button
            ...
            className="absolute top-2 right-10 p-1 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors opacity-0 group-hover:opacity-100 z-10 disabled:opacity-50 disabled:cursor-not-allowed"
            title="Delete Group"
          >
  ...
  return (
    <div className="p-4 bg-gray-50 min-h-screen">
      ...
      <DeleteConfirmationDialog
        ...
        isDeleting={deletingGroupId !== null}
      />
